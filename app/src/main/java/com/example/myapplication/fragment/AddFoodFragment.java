...
public class AddFoodFragment extends Fragment {
    ...
    private void addfoodpost(BaiDang bd){
        String url = "https://sg87k2rd-3000.asse.devtunnels.ms/api/nguoidung/add/" + user.getId();
                ...
                response -> {
                    try {
                        ...
                        if (status.equals("success")) {
                            ...
                            if (getActivity() instanceof HomeFragment.OnFoodItemSelectedListener) {
                                if(response.has("data")) {
                                    JSONObject data = response.getJSONObject("data");
                                    bd.setId(data.optString("_id"));
                                }
                                ((HomeFragment.OnFoodItemSelectedListener) getActivity()).onFoodItemSelected(bd);
                            }
                        } else {
